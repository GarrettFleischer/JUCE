...
namespace juce
{
    using createUnityPeerFunctionType = ComponentPeer* (*)(Component&);
    ...
    public:
        ...
        void fillPixels()
        {
            ...
            renderer.addTransform(AffineTransform::verticalFlip(static_cast<float>(getComponent().getHeight())));
        ...
        void forwardMouseEvent(Point<float> position, ModifierKeys mods)
        {
            ...
            handleMouseEvent(MouseInputSource::mouse, position, mods, MouseInputSource::defaultPressure,
                             MouseInputSource::defaultOrientation, Time::currentTimeMillis());
        }
    ...
    private:
        ...
        struct UnityBitmapImage : public ImagePixelData
        {
                                      ...
                                      Image::BitmapData::ReadWriteMode mode) override
            {
                ...
                const auto offset = static_cast<size_t>(x) * static_cast<size_t>(pixelStride) + static_cast<size_t>(y) *
                    static_cast<size_t>(lineStride);
                bitmap.data = imageData + offset;
                bitmap.size = static_cast<size_t>(lineStride * height) - offset;
            ...
            Ptr clone() override
            {
                ...
                for (int i = 0; i < height; ++i)
                    memcpy(im->imageData + i * lineStride, imageData + i * lineStride, static_cast<size_t>(lineStride));
        ...
        KeyPress getKeyPress(int keyCode, String name)
        {
            ...
            if (keyCode >= 256 && keyCode <= 265)
                return {
                    KeyPress::numberPad0 + (keyCode - 256), ModifierKeys::currentModifiers,
                    String(keyCode - 256).getCharPointer()[0]
                };
    ...
    public:
        ...
        pluginInstance->setPlayConfigDetails (configs[0][0], configs[0][1], state->sampleRate, samplesPerBlock);
            ...
            scratchBuffer.setSize(jmax(pluginInstance->getTotalNumInputChannels(),
                                       pluginInstance->getTotalNumOutputChannels()), samplesPerBlock);
        }
        ...
        void process(float* inBuffer, float* outBuffer, int bufferSize, int numInChannels, int numOutChannels,
                     bool isBypassed)
        {
            ...
            for (int pos = 0; pos < bufferSize;)
            {
                ...
                processBuffers(inBuffer + (pos * numInChannels), outBuffer + (pos * numOutChannels), max, numInChannels,
                               numOutChannels, isBypassed);
        ...
        void declareParameters(UnityAudioEffectDefinition& definition)
        {
            ...
            {
                numParams = static_cast<int>(juceParameters.size());

                parametersPtr.reset(static_cast<UnityAudioParameterDefinition*>(std::calloc(
                ...
                for (int i = 0; i < numParams; ++i)
                {
                    ...
                    const auto nameLength = static_cast<size_t>(numElementsInArray(paramDef.name));
                    const auto unitLength = static_cast<size_t>(numElementsInArray(paramDef.unit));

                    parameter->getName(static_cast<int>(nameLength) - 1).copyToUTF8(paramDef.name, nameLength);
        ...
        {
            juceParameters.getParamForIndex(index)->setValueNotifyingHost(value);
        }
        ...
        Juce4UnityAudioProcessor* getInstance() const
        {
            return dynamic_cast<Juce4UnityAudioProcessor*>(pluginInstance.get());
        }

    private:
        //==============================================================================
        void processBuffers(float* inBuffer, float* outBuffer, int bufferSize, int numInChannels, int numOutChannels,
                            bool isBypassed)
        {
            int ch;
            for (ch = 0; ch < numInChannels; ++ch)
            {
                using DstSampleType = AudioData::Pointer<
                    AudioData::Float32, AudioData::NativeEndian, AudioData::NonInterleaved, AudioData::NonConst>;
                using SrcSampleType = AudioData::Pointer<
                    AudioData::Float32, AudioData::NativeEndian, AudioData::Interleaved, AudioData::Const>;
            ...
            {
                using DstSampleType = AudioData::Pointer<
                    AudioData::Float32, AudioData::NativeEndian, AudioData::Interleaved, AudioData::NonConst>;
                using SrcSampleType = AudioData::Pointer<
                    AudioData::Float32, AudioData::NativeEndian, AudioData::NonInterleaved, AudioData::Const>;
    ...
    static OwnedArray<AudioProcessorUnityWrapper> unityWrappers;
    ...
    static UnityAudioEffectDefinition getEffectDefinition()
    {
        ...
        name.copyToUTF8(result.name, static_cast<size_t>(numElementsInArray(result.name)));
        ...
        result.channels = (wrapper->getNumInputChannels() != 0
                               ? 0
                            ...
                            int numOutChannels)
        {
            ...
            if (pluginInstance != nullptr)
            {
                ...
                pluginInstance->process(inBuffer, outBuffer, static_cast<int>(bufferSize), numInChannels,
                                        numOutChannels, bypassed);
            }
            else
            {
        ...
        result.getFloatBuffer = [](UnityAudioEffectState* state, const char* kind, float* buffer, int numSamples)
        {
            ...
            else if (kindStr == StringRef("ID"))
            {
                ...
                for (HashMap<int, AudioProcessorUnityWrapper*>::Iterator i(getWrapperMap()); i.next();)
                {
                    ...
                    {
                        buffer[0] = static_cast<float>(i.getKey());
                        break;
                    }
                }
            ...
            else if (kindStr == StringRef("Size"))
            {
                ...
                buffer[0] = static_cast<float>(editor.getBounds().getWidth());
                buffer[1] = static_cast<float>(editor.getBounds().getHeight());
                buffer[2] = static_cast<float>(editor.getConstrainer()->getMinimumWidth());
                buffer[3] = static_cast<float>(editor.getConstrainer()->getMinimumHeight());
                buffer[4] = static_cast<float>(editor.getConstrainer()->getMaximumWidth());
                buffer[5] = static_cast<float>(editor.getConstrainer()->getMaximumHeight());
            }
...
UNITY_INTERFACE_EXPORT int UNITY_INTERFACE_API UnityGetAudioEffectDefinitions(
...
static juce::ModifierKeys unityModifiersToJUCE(UnityEventModifiers mods, bool mouseDown, int mouseButton = -1)
{
    ...
    if ((mods & shift) != 0) flags |= juce::ModifierKeys::shiftModifier;
    if ((mods & control) != 0) flags |= juce::ModifierKeys::ctrlModifier;
    if ((mods & alt) != 0) flags |= juce::ModifierKeys::altModifier;
    if ((mods & command) != 0) flags |= juce::ModifierKeys::commandModifier;
...
UNITY_INTERFACE_EXPORT void UNITY_INTERFACE_API unityMouseDown(int id, float x, float y, UnityEventModifiers unityMods,
                                                               int button)
{
    getWrapperChecked(id)->getEditorPeer().forwardMouseEvent({x, y}, unityModifiersToJUCE(unityMods, true, button));
}

UNITY_INTERFACE_EXPORT void UNITY_INTERFACE_API unityMouseDrag(int id, float x, float y, UnityEventModifiers unityMods,
                                                               int button)
{
    getWrapperChecked(id)->getEditorPeer().forwardMouseEvent({x, y}, unityModifiersToJUCE(unityMods, true, button));
}
...
UNITY_INTERFACE_EXPORT void UNITY_INTERFACE_API unityKeyEvent(int id, int code, UnityEventModifiers mods,
                                                              const char* name)
{
...
{
    getWrapperChecked(id)->getEditorPeer().getEditor().setBounds({
...
void samplerLoadInstrument(const char* path, const int len)
{
    for (const auto wrapper : juce::unityWrappers)
    {
        if (const auto instance = wrapper->getInstance())
        {
            instance->loadInstrument(juce::String(path, len));
        }
    }
}
...
void samplerUnloadInstrument(const char* path, int len)
{
    for (const auto wrapper : juce::unityWrappers)
    {
        if (const auto instance = wrapper->getInstance())
        {
            instance->unloadInstrument(juce::String(path, len));
        }
    }
}
...
void samplerSetInstrument(const char* path, int len)
{
    for (const auto wrapper : juce::unityWrappers)
    {
        if (const auto instance = wrapper->getInstance())
        {
            instance->setInstrument(juce::String(path, len));
        }
    }
}
